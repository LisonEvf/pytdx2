...
### 1.2 新增工具（基于现有parser）
- [x] **stock_detail** - 个股详情（报价+F10摘要）✅ 已完成
  - 调用: get_quotes + get_company_info
  - 返回: 价格、涨跌幅、市盈率、行业、概念
  
- [x] **capital_flow** - 资金流向（估算版）✅ 已完成
  - 基于大单成交估算主力资金
  - 调用: get_transaction
  - 返回: 大单净流入、中单、小单统计
...
- [x] **hot_concepts** - 热门概念板块 ✅ 已完成
  - 基于涨幅和成交量筛选
  - 调用: get_stock_top_board
  - 返回: 涨幅前10概念板块 + 热度评分
