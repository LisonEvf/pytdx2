...
class f452(BaseParser):
    ...
    def deserialize(self, data):
        ...
        for i in range(count):
            market, code_num, p1, p2 = struct.unpack(
                "<BIff", data[i * 13 + 2 : i * 13 + 15]
            )
            result.append(
                {"market": MARKET(market), "code": f"{code_num}", "p1": p1, "p2": p2}
            )
...
class TODO547(BaseParser):
    def __init__(self, stocks: list[MARKET, str]):
        ...
        for market, code in stocks:
            self.body.extend(
                struct.pack("<B6sHH", market.value, code.encode("gbk"), 22234, 2)
            )
