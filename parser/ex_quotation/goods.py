...
class CategoryList(BaseParser):
    ...
    def deserialize(self, data):
        ...
        for i in range(count):
            market, name, code, abbr = struct.unpack(
                "<B32sB30s", data[64 * i + 2 : 64 * i + 66]
            )
            result.append(
                {
    ...
    def deserialize(self, data):
        ...
        for i in range(count):
            (
                ...
                u13,
                u14,
            ) = struct.unpack("<BBBH9s26sffHHHHHHHH", data[i * 64 + 6 : i * 64 + 70])
...
def unpack_futures(data, code_len: int = 23):
    ...
    (
        ...
        u14,
        hold_position,
    ) = struct.unpack(f"<I5f4If4I", data[1 + code_len : 61 + code_len])
    pending_list = struct.unpack("<5f5I5f5I", data[61 + code_len : 141 + code_len])
    pending = {
        "bids": [
            {"price": pending_list[i], "vol": pending_list[i + 5]} for i in range(5)
        ],
        "asks": [
            {"price": pending_list[i], "vol": pending_list[i + 5]}
    ...
    (
        ...
        u6,
        pre_close_price,
    ) = struct.unpack("<HfIffIIIIf", data[141 + code_len : 179 + code_len])
    (
        s1,
        pre_vol,
        u7,
        ...
        u11,
        u12,
    ) = struct.unpack("<12sff12sff25sfIIff24sHB", data[179 + code_len : 291 + code_len])

    # 当没有 date_raw 数据时,会报错
    # goods.Futures_QuotesList(ExtMarketCategory.港股.value, 1895, 2)  02632没有date_raw数据
    if date_raw // 10000 == 0:
        date_str = date(1900, 1, 1)
    else:
        date_str = date(date_raw // 10000, date_raw % 10000 // 100, date_raw % 100)
...
class K_Line(BaseParser):
    ...
    def deserialize(self, data):
        ...
        for _ in range(count):
            date_num, open, high, low, close, amount, vol, _ = struct.unpack(
                "<IfffffIf", data[20 + 32 * _ : 20 + 32 * _ + 32]
            )
...
class HistoryTransaction(BaseParser):
    def __init__(self, category: EX_CATEGORY, code: str, date: date):
        ...
        self.body = struct.pack(
            "<IB43sH", date, category.value, code.encode("gbk"), 0x78
        )

    @override
    def deserialize(self, data):
        category, name, date, start_price, _, _, count = struct.unpack(
            "<B39sIfIIH", data[:58]
        )

        results = []
        for _ in range(count):
            minutes, price, vol, u, buy_sell = struct.unpack(
                "<HIIIH", data[58 + 16 * _ : 58 + 16 * _ + 16]
            )
            results.append(
                {
    ...
    def __init__(self, start: int = 0, mode: int = 1):
        self.body = bytearray(
            struct.pack(
                ...
                start,
                0,
                bytes.fromhex("00781f0e6a37447b502b7c0d01404c0a"),
                mode,
            )
        )
...
class f2487(BaseParser):
    ...
    def deserialize(self, data):
        ...
        active, pre_close, open, high, low, close, u1, price = struct.unpack(
            "<I7f", data[24:56]
        )
...
class f2488(BaseParser):
    def __init__(self, category: EX_CATEGORY, code: str):
        self.body = struct.pack(
            "<B23sIHII", category.value, code.encode("gbk"), 0, 55, 0, 0
        )
...
class K_Line_2(BaseParser):
    ...
    @override
    def deserialize(self, data):
        category, name, period, times, start, _, _, count = struct.unpack(
            "<B23sHHIIIH", data[:42]
        )
        ...
        for i in range(count):
            date_num, open, high, low, close, amount, vol, _ = struct.unpack(
                "<IfffffII", data[42 + 32 * i : 42 + 32 * i + 32]
            )
...
class TickChart(BaseParser):
    ...
    def deserialize(self, data):
        ...
        charts = []
        for i in range(count):
            minutes, price, avg, vol, _ = struct.unpack(
                "<HffII", data[i * 18 + 34 : i * 18 + 52]
            )
            charts.append(
                {
                    "time": time(minutes // 60 % 24, minutes % 60),
...
class HistoryTickChart(BaseParser):
    def __init__(self, category: EX_CATEGORY, code: str, date: date):
        date = date.year * 10000 + date.month * 100 + date.day
        self.body = struct.pack(
            "<IB23s6sH", date, category.value, code.encode("gbk"), b"", 0
        )

    @override
    def deserialize(self, data):
        category, name, date, avg_price, _, _, count = struct.unpack(
            "<B23sIfIIH", data[:42]
        )
        charts = []
        for i in range(count):
            minutes, price, avg, vol, u = struct.unpack(
                "<HffII", data[42 + 18 * i : 42 + 18 * i + 18]
            )
            charts.append(
                {
                    "time": time(minutes // 60 % 24, minutes % 60),
...
class ChartSampling(BaseParser):
    ...
    @override
    def deserialize(self, data):
        category, code, a, b, c, d, e, f, g, h, count = struct.unpack(
            "<H22s9H", data[:42]
        )
...
class f2562(BaseParser):
    ...
    def deserialize(self, data):
        ...
        for i in range(count):
            category, name, u, index, switch, u2, u3, u4, u5, u6 = struct.unpack(
                "<H23sHIBfffHH", data[48 * i + 2 : 48 * i + 50]
            )
            result.append(
                {
                    "name": name.decode("gbk").replace("\x00", ""),
