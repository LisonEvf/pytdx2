...
class CommonClientMixin:
    ...
    @require_sp_mode
    @update_last_ack_time
    def get_board_members_quotes(self, board_symbol: str, count=10000, filter=0):
        MAX_LIST_COUNT = 80
        security_list = []
        ...
        for start in range(0, count, MAX_LIST_COUNT):
            ...
            rs = self.call(BoardMembersQuotes(board_symbol=board_symbol, start=start, page_size=current_count, filter=filter))
            part = rs["stocks"]
